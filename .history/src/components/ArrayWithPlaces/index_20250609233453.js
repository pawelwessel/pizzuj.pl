...
const isOpenNow = (weekdayText) => {
  ...
  const currentDay = moment().locale("pl").format("dddd").toLowerCase();
  const currentTime = moment();

  // Find today's schedule
  const todaySchedule = weekdayText.find((schedule) =>
    schedule.toLowerCase().startsWith(currentDay)
  );
  ...
  let openTime = moment(start, "HH:mm");
  let closeTime = moment(end, "HH:mm");
