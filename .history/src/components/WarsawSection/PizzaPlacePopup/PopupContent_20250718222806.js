...
export default function PopupContent({ place }) {
  ...
  return (
    <div className="h-[60vh] overflow-y-auto p-6 lg:p-8 bg-white rounded-2xl shadow-md">
      {/* Name and rating */}
      <div className="flex flex-col lg:flex-row md:justify-between gap-4 border-b pb-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2 text-gray-500">
            ...
            <span className="uppercase font-cocosharp font-light text-base">
              {place.city}
            </span>
          </div>
        </div>
      ...
      {/* Contact & info */}
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 border-b">
      ...
      {/* Recent opinions */}
      <div className="mt-6">
        <h3 className="font-heading text-lg font-semibold text-zinc-800 mb-3">
          Ostatnie opinie
        </h3>
        {Array.isArray(recentOpinions) && recentOpinions.length > 0 ? (
          <div className="space-y-4">
            {recentOpinions.slice(0, 5).map((op, idx) => (
              <div
                key={idx}
                className="bg-gray-50 rounded-xl p-4 border hover:bg-gray-100 transition-colors duration-200"
              >
