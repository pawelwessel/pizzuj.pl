...
import { FaPhone, FaGlobe, FaClock, FaStar, FaMoneyBillWave } from "react-icons/fa6";
...
function getPriceLevel(level) {
  ...
  const euro = "€".repeat(level);
  return `${euro} — ${levels[idx].label}`;
}
...
export default function PopupContent({ place }) {
  ...
  const user_ratings_total = details.user_ratings_total || place.user_ratings_total;
  ...
  const recentOpinions = place.recentOpinions || place.recent_opinions || place.recent_opinion || [];

  // Quixy style: clean, card-like, clear sections, subtle dividers, icons, modern
  return (
    <div className="p-6 lg:p-8 bg-white rounded-2xl shadow-md">
      {/* Name and rating */}
      <div className="flex flex-col lg:flex-row md:justify-between gap-4 border-b pb-4">
        <div>
          <h2 className="font-heading text-2xl lg:text-3xl font-bold text-gray-900">{name}</h2>
          <div className="flex items-center gap-2 mt-2 text-gray-500">
            <MdLocationPin className="w-5 h-5 text-primary-500" />
            <span className="uppercase font-cocosharp font-light text-base">{place.city}</span>
          </div>
        </div>
        <div className="flex items-center lg:flex-col gap-3 mt-2">
          ...
          {user_ratings_total && (
            <span className="text-gray-500 text-sm">({user_ratings_total} ocen)</span>
          )}
          </div>
          {price_level && (
       ...
       {photos && photos.length > 0 && (
         <div className="mt-4">
           <h3 className="font-heading text-lg font-semibold text-gray-800 mb-3">Galeria zdjęć</h3>
           <div className="flex gap-2 overflow-x-auto pb-2">
             {photos.map((photo, idx) => {
               const src = photo.photo_reference
                 ? `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photo_reference=${photo.photo_reference}&key=${process.env.NEXT_PUBLIC_FIREBASE_MAP_KEY}`
                 : "/assets/pizza.png";
               return (
                 <div 
                   key={idx} 
                   className="relative w-32 h-24 rounded-lg overflow-hidden flex-shrink-0 border cursor-pointer hover:scale-105 transition-transform duration-200 shadow-md hover:shadow-lg"
                   onClick={() => {
      ...
      {/* Contact & info */}
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 border-b pb-6">
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <span className="text-gray-600 text-sm">{address || "Brak informacji"}</span>
            </div>
          </div>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <span className="text-gray-600 text-sm">{phone || "Brak informacji"}</span>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <FaGlobe className="w-5 h-5 text-primary-500 mt-1" />
            <div>
              <span className="block font-medium text-gray-700">Strona WWW</span>
              {website ? (
                <a
                  href={website}
                  target="_blank"
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="block font-medium text-gray-700">Godziny otwarcia</span>
          ...
          {geometry && geometry.location && (
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="block font-medium text-gray-700">Lokalizacja (mapa)</span>
                <a
             ...
             {/* Recent opinions */}
       <div className="mt-6">
         <h3 className="font-heading text-lg font-semibold text-zinc-800 mb-3">Ostatnie opinie</h3>
         {Array.isArray(recentOpinions) && recentOpinions.length > 0 ? (
           <div className="space-y-4 max-h-64 overflow-y-auto">
             {recentOpinions.slice(0, 5).map((op, idx) => (
               <div key={idx} className="bg-gray-50 rounded-xl p-4 border hover:bg-gray-100 transition-colors duration-200">
                 <div className="flex items-center gap-2 mb-2">
                   <div className="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center">
                     <span className="text-primary-700 font-semibold text-sm">
                       {op.author_name ? op.author_name.charAt(0).toUpperCase() : 'U'}
                     </span>
                   </div>
                   <div className="flex-1">
                     <span className="font-semibold text-gray-700 text-sm">{op.author_name || 'Anonimowy'}</span>
                     {op.rating && (
                       <div className="flex items-center gap-1 mt-1">
                         <span className="flex items-center gap-1 text-yellow-500 text-xs">
                           {[...Array(5)].map((_, index) => (
                             <FaStar 
                               key={index} 
                               className={`w-3 h-3 ${index < op.rating ? 'text-yellow-500' : 'text-gray-300'}`} 
                             />
                           ))}
                         </span>
                 ...
                 <p className="text-gray-700 text-sm leading-relaxed">{op.text}</p>
               </div>
             ))}
           ...
           <div className="text-center py-8">
             <div className="text-gray-400 text-sm">
               ...
               <p className="text-xs mt-1">Bądź pierwszym, który oceni to miejsce!</p>
             </div>
           </div>
         )}
       </div>
