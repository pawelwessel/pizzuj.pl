...
export default function PricingCard({
  ...
  return (
    <div
      className={`group relative flex flex-col p-8 xl:p-10 rounded-3xl border transition-all duration-700 transform hover:-translate-y-4 ${
        isPopular
    ...
    >
      ...
      <div
        className={`relative ${
          isPopular || isEnterprise ? "bg-white rounded-3xl p-8 xl:p-10" : ""
        }`}
      >
        {isPopular && (
        ...
        >
          <div className="relative inline-block mb-6">
            ...
            >
              <IconComponent
                className={`${colorMap[iconColor]} text-5xl transition-transform duration-300 group-hover:scale-110`}
              />
            </div>
            <div
              ...
              <>
                <div
                  className={`text-5xl xl:text-6xl font-bold mb-2 ${
                    isPopular
                      ? "bg-gradient-to-r from-orange-600 to-[#ffa920] bg-clip-text text-transparent"
                      : isEnterprise
            ...
            <li key={index} className="flex items-start group/item">
              <div className="min-w-6 h-6 mt-1.5">
                {feature.icon ? (
                  <feature.icon
                    className={`${
                      colorMap[feature.iconColor] || "text-green-500"
                    } w-5 h-5 transition-transform duration-200 group-hover/item:${
                      feature.animation || "translate-x-1"
                    }`}
                  />
                ) : (
        ...
        <Link
          href={ctaLink}
          className={`mx-auto block w-max max-w-full font-cocosharp text-center font-semibold py-4 px-6 rounded-2xl transition-all duration-300 border-2 transform hover:scale-105 relative overflow-hidden group ${
            isPopular
        ...
        >
          ...
          <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            ...
            <div
              className="absolute top-4 right-6 w-0.5 h-0.5 bg-white rounded-full animate-ping"
            ...
            ></div>
            <div
              className="absolute bottom-6 right-4 w-0.5 h-0.5 bg-white rounded-full animate-ping"
            ...
            ></div>
            <div
              className="absolute top-1/3 right-2 w-1 h-1 bg-white rounded-full animate-ping"
              style={{ animationDelay: "1s" }}
            ></div>
            <div
              className="absolute top-1/4 left-6 w-0.5 h-0.5 bg-white rounded-full animate-ping"
              style={{ animationDelay: "1.2s" }}
            ></div>
            <div
              className="absolute bottom-1/4 right-8 w-1 h-1 bg-white rounded-full animate-ping"
