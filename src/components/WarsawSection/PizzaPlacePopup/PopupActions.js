    ...
    <div className="px-6 lg:px-8 pb-6 lg:pb-8">
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <a
            href={`/pizza/${createLinkFromText(place.city)}/${createLinkFromText(place.name)}`}
            className="flex-1 bg-primary-600 hover:bg-primary-700 text-white font-semibold py-4 px-6 rounded-2xl transition-all duration-300 flex items-center justify-center gap-3 text-base lg:text-lg shadow-lg hover:shadow-xl transform hover:scale-105"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          {place.formatted_address && (
            <a
              href={`https://maps.google.com/?q=${encodeURIComponent(place.formatted_address)}`}
              target="_blank"
              rel="noopener noreferrer"
        ...
        {place.price_level && (
          <div className="text-center pt-2">
            <span className="text-sm text-gray-500">
              Poziom cenowy: {"💰".repeat(place.price_level)}
            </span>
          </div>
        )}
      </div>
