...
const PizzeriaPromotions = ({ pizzeriaId }) => {
  ...
  const loadPromotions = async () => {
    try {
      setLoading(true);
      const items = await getPromotions(pizzeriaId);
      setPromotions(items);
    } catch (error) {
      console.error('Error loading promotions:', error);
      toast.error('Błąd podczas ładowania promocji');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreatePromotion = async () => {
    ...
    try {
      ...
      const promotionData = {
        ...
        maxUsage: parseInt(newPromotion.maxUsage) || 100,
        status: 'active'
      };

      const newPromotionItem = await createPromotion(pizzeriaId, promotionData);
      setPromotions([...promotions, newPromotionItem]);
      ...
      toast.error('Błąd podczas tworzenia promocji');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdatePromotion = async () => {
    if (!selectedPromotion?.id) {
      toast.error('Brak ID promocji do edycji');
      return;
    }
    ...
    try {
      ...
      const updateData = {
        ...selectedPromotion,
        value: parseFloat(selectedPromotion.value) || 0,
        maxUsage: parseInt(selectedPromotion.maxUsage) || 100
      };

      await updatePromotion(selectedPromotion.id, updateData);
      
      setPromotions(promotions.map(item => 
        item.id === selectedPromotion.id ? { ...item, ...updateData } : item
      ));
  ...
  const handleToggleStatus = async (promotionId, currentStatus) => {
    try {
      ...
      await togglePromotionStatus(promotionId, newStatus);
      setPromotions(promotions.map(item => 
        item.id === promotionId ? { ...item, status: newStatus } : item
      ));
      toast.success(`Promocja została ${newStatus === 'active' ? 'aktywowana' : 'zaplanowana'}!`);
    } catch (error) {
      console.error('Error toggling promotion status:', error);
      toast.error('Błąd podczas zmiany statusu promocji');
    }
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <button
            ...
            title={promotion.status === 'active' ? 'Zaplanuj' : 'Aktywuj'}
          >
            <FaClock />
          </button>
          <button
        ...
        >
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {(newPromotion.type === 'discount' || newPromotion.type === 'fixed_discount') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {newPromotion.type === 'discount' ? 'Wartość zniżki (%)' : 'Wartość zniżki (zł)'}
                  </label>
                  <input
                    type="number"
                    min="0"
                    max={newPromotion.type === 'discount' ? "100" : undefined}
            ...
            <div className="flex justify-end space-x-4 mt-6">
              ...
              <button
                ...
                disabled={saving}
                className="px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg hover:from-orange-600 hover:to-orange-700 transition-all duration-200 flex items-center disabled:opacity-50"
              >
    ...
    <AnimatePresence>
      {showEditModal && selectedPromotion && (
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-gray-800">Edytuj promocję</h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {promotionTypes.map(type => (
                    <button
                      key={type.id}
                      onClick={() => setSelectedPromotion({...selectedPromotion, type: type.id})}
                      className={`p-3 rounded-lg border-2 flex flex-col items-center transition-colors ${
                        selectedPromotion.type === type.id 
                          ? 'border-orange-500 bg-orange-50 text-orange-600' 
                          : 'border-gray-200 hover:border-gray-300'
                      }`}
                    >
              ...
              {(selectedPromotion.type === 'discount' || selectedPromotion.type === 'fixed_discount') && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {selectedPromotion.type === 'discount' ? 'Wartość zniżki (%)' : 'Wartość zniżki (zł)'}
                  </label>
                  <input
                    type="number"
                    min="0"
                    max={selectedPromotion.type === 'discount' ? "100" : undefined}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setSelectedPromotion({...selectedPromotion, maxUsage: parseInt(e.target.value)})}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
                />
              </div>
            </div>
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">Ładowanie promocji...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-xl shadow-lg">
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            ...
            >
              <FaBullhorn />
              <span>Aktywne ({activePromotions.length})</span>
            </button>
            <button
            ...
            >
              <FaClock />
              <span>Zaplanowane ({scheduledPromotions.length})</span>
            </button>
          </nav>
        </div>
        ...
        <div className="p-6">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="space-y-6"
            >
              {activeTab === 'active' && activePromotions.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {activePromotions.map(renderPromotionCard)}
                </div>
              )}
              
              {activeTab === 'scheduled' && scheduledPromotions.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {promotions.reduce((sum, p) => sum + (p.usage || 0), 0)}
              </p>
            </div>
