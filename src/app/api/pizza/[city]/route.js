...
export async function GET(params, req) {
  try {
    ...
    if (!searchData.results || searchData.results.length === 0) {
      return NextResponse.json({ error: "No pizza places found in this city" }, { status: 404 });
    }
    ...
    return NextResponse.json({ error: "Failed to fetch pizza places data" }, { status: 500 });
  }
}